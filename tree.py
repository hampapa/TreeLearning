...
def generate_sample(n):
    # Observation: the way this function is implemented  leads to
    # Y which always contains perfect decisions. The data
    # set as a result does not contain any noise which is not
    # reflecting most real world scenarios in decision making.
    # As this function is used for the creation of the training set as
    # well as the test set the accuracy of the model is very high also
    # with very few training samples only
...
class DecisionTree:
    def __init__(self, parent, name, clfn=None):
        ...
        self.name = name
        self.child = {}
        # clnf holds the classification in case the node is a leave
...
class DecisionTreeClassifier:
    ...
    def dtl(self, x, y, A, pex, pey):
        ...
        if (len(x.index) == 0):
            ...
            dt = DecisionTree(None,"clfn",pv_clfn)
            return dt
        # else if all examples have the same classification return the
        # classification
        elif y.nunique() == 1:
            clfn = y.iloc[0]
            dt = DecisionTree(None,"clfn",clfn)
            return dt
...
def main():
    ...
    s_test_df = generate_sample(12000)
    x_test_df = s_test_df.iloc[:,:-1]
    y_test = s_test_df.iloc[:,-1]
    y_pred = dtree.predict(x_test_df)
    accy = (y_pred == y_test)
    print(accy.sum()/len(accy))
    
    # Learning: when training set does not contain all edges of the population
    # data set the prediction function runs into a KeyError. This can happen
